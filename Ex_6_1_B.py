...
'''6_B) Add to the game loop the same possibility to create different-sized objects 
using the right mouse button as you already did in Exercise 4.1 c). This allows us
to see what happens when nonidentical objects collide.'''
...
class Environment:
    ...
    def check_boundary_collisions(self):
        ...
        for p in self.particles:
            ...
            if dist_right > self.collision_tolerance and p.vel_new[0] > self.collision_velocity_tolerance:
                colliding_particles.append((p, np.array([1.0, 0.0]), dist_right))

            if dist_left > self.collision_tolerance and p.vel_new[0] < -self.collision_velocity_tolerance:
                colliding_particles.append((p, np.array([-1.0, 0.0]), dist_left))
            ...
            if dist_bottom > self.collision_tolerance and p.vel_new[1] < -self.collision_velocity_tolerance:
                colliding_particles.append((p, np.array([0.0, -1.0]), dist_bottom))

            

        return colliding_particles

    #Checking boundry and draw inside only

    def draw_inside_boundry(self, radius, x, y):
        pos_new_partical = np.array([x,y])
        # Calculate the distance from the right edge
        # If dist_right >= 0, the object has penetrated into the wall
        dist_right = x + radius - self.width / 2
        # Do the same for all the other walls
        dist_left = -x + radius - self.width / 2
        dist_top = y + radius - self.height / 2
        dist_bottom = -y + radius - self.height / 2
    ...
    # Check whether any of the particles collide with each other
    def collision_Check(self, radius, x, y):
        num_particles = len(self.particles)
        pos_new_partical = np.array([x,y])
        # Loop over all the pairs of particles
        for i in range(num_particles):
            p_1 = self.particles[i]            
                # Calculate the relative position between the particles
                # And using this, also the distance and the surface normal
            rel_pos = pos_new_partical - p_1.pos_new
            dist = np.linalg.norm(rel_pos)               
            if dist < radius + p_1.radius:
                return False
        return True    
    ...
    def add_particle(self, pos, vel=[0.0, 0.0], radius=0.5, mass=1):
        self.particles.append(Particle(mass=mass,radius=radius, pos=pos, vel=vel))
...
while running:
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.QUIT:
            running = False
 #Create a particle with mouse(lef Click == 1)
        ...
        if event.type == pygame.MOUSEBUTTONUP and event.button == 1:
            ...
            #format of the add particle     self, pos, vel=[0.0, 0.0], radius=0.5, mass=1):
            if env.collision_Check(0.5, x, y) and env.draw_inside_boundry(0.5,x,y):
                env.add_particle([x, y], [v_x, v_y])

        
        #Create a particle with mouse(left Click == 3)
        ...
        if event.type == pygame.MOUSEBUTTONUP and event.button == 3:
            ...
            v_y = -(y_screen_start - y_screen_end) * 0.03
            x, y = env.screen_to_env(x_screen_start, y_screen_start)
            # also giving the parameter in correct sequence that add_particle has (x,y,..... mass, radius)
            #format of the add particle add_particle(self, x, y, v_x = 0, v_y=0, m = 1, radius= 0.5):
            if env.collision_Check(0.25, x, y) and env.draw_inside_boundry(0.25,x,y):
                env.add_particle([x,y],[v_x,v_y],0.25,2)
